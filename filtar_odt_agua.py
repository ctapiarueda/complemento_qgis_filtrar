...
class FiltroPorSmoNombreFechaYDepartamentos(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.OUTPUT, 'Filtro Odt Agua')
        )

    def processAlgorithm(self, parameters, context, feedback):
        layers = QgsProject.instance().mapLayersByName('vw_gra_odt_Agua')
        if not layers:
          raise QgsProcessingException("La capa 'odt_agua' no está cargada en el proyecto.")
        capa = layers[0]
    ...
    def name(self):
        return 'filtro_odt_agua'

    def displayName(self):
        return 'Filtrar odt de Agua - OSSE'
    ...
    def shortHelpString(self):
        return 'Filtra las Odt (Ordenes de trabajo) de Agua por Motivo/s (SmoNombre), Fecha desde (sin hora)(campo_fecha), Fecha hasta (sin hora)(campo_fecha) y  Departamento/s de San Juan (DepId). Realizado en OSSE- Departamento TIC- Área Sistemas- Sector SIG por Carlos Tapia '
