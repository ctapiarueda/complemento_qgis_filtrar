...
class FiltroPorSmoNombreFechaYDepartamentos(QgsProcessingAlgorithm):
    ...
    campos_fecha = ['FechaOriginada', 'FechaIniciada', 'FechaFinalizada']
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
             QgsProcessingParameterEnum(
                self.FECHA_CAMPO,
                'Campo de fecha a utilizar',
                options=self.campos_fecha,
                defaultValue=0  # por defecto FechaOrigen
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.OUTPUT, 'Filtro Odt Cloaca')
        )

    def processAlgorithm(self, parameters, context, feedback):
        layers = QgsProject.instance().mapLayersByName('vw_gra_odt_cloaca')
        if not layers:
          raise QgsProcessingException("La capa 'odt_cloaca' no está cargada en el proyecto.")
        capa = layers[0]
        ...
        indice_fecha = self.parameterAsEnum(parameters, self.FECHA_CAMPO, context)
        campo_fecha = self.campos_fecha[indice_fecha]
    ...
    def name(self):
        return 'filtro_odt_cloaca'

    def displayName(self):
        return 'Filtrar odt de Cloaca - OSSE'
    ...
    def shortHelpString(self):
        return 'Filtra las Odt (Ordenes de trabajo) de Cloaca por Motivo/s (SmoNombre), Fecha desde (sin hora)(campo_fecha), Fecha hasta (sin hora)(campo_fecha) y  Departamento/s de San Juan (DepId). Realizado en OSSE- Departamento TIC- Área Sistemas- Sector SIG por Carlos Tapia '
